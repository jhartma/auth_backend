...
async function createUser({ body: { username, email, password } }, res) {
    ...
    const checkUsername = await db_1.Accounts.findOne({ $and: [
        ...
        if (err) {
            logger_1.default.log("error", `[ checkUsername ] An error occurred when looking for account for username: ${uname}. Err: ${err}`);
    ...
    const checkEmail = await db_1.Accounts.findOne({ emails: { $elemMatch: { address: mail } } }, (err) => {
        if (err) {
            logger_1.default.log("error", `[ checkEmail ] An error occurred when looking for account for email: ${mail}. Err: ${err}`);
            res.json({ status: 500, message: messages_1.MESSAGE_FAILURE_UNDEFINED, data: { error: err } });
            return;
        }
    });
    if (checkUsername) {
        logger_1.default.log("info", `Account for username ${uname} already exists`);
        res.json({ status: 503, message: messages_1.MESSAGE_FAILURE_USER_EXISTS });
        return;
    }
    if (checkEmail) {
        logger_1.default.log("info", `Account for email ${mail} already exists`);
        res.json({ status: 504, message: messages_1.MESSAGE_FAILURE_EMAIL_EXISTS });
        return;
    }
    if (pw.length < 6) {
        logger_1.default.log("info", `Password is too short: ${pw}`);
        res.json({ status: 505, message: messages_1.MESSAGE_FAILURE_PASSWD_TOO_SHORT });
        return;
    }
    if (!regex_1.passwordRegex.test(pw)) {
        logger_1.default.log("info", `Invalid password: ${pw}`);
        res.json({ status: 506, message: messages_1.MESSAGE_FAILURE_PASSWD_INSECURE });
        return;
    }
    if (!isEmail.validate(mail, { checkDNS: false })) {
        logger_1.default.log("info", `Invalid email: ${mail}`);
    ...
    const validationToken = await createValidationToken_1.createValidationToken().catch((err) => {
        logger_1.default.log("error", `[ createValidationToken ] Could't generate validation token: ${err}`);
        res.json({ status: 500, message: messages_1.MESSAGE_FAILURE_UNDEFINED, data: { error: err } });
        return null;
    });
    ...
    if (createdUser) {
        ...
        sendMail_1.sendMail(user.emails[0].address, subject, message, { SMTP_PASSWORD: config_1.SMTP_PASSWORD, SMTP_USER: config_1.SMTP_USER, SMTP_SERVER: config_1.SMTP_SERVER, SMTP_PORT: config_1.SMTP_PORT, EMAIL_ADDRESS: config_1.EMAIL_ADDRESS })
            .catch((err) => {
            logger_1.default.log("error", `Could't send email: ${err}`);
